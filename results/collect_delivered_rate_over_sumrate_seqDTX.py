...
Collect delivered rates from folders and create csv for plotting
y-axis: delivered rate for several techniques.
x-axis: target rate
...
def main(searchpath, outpath):
        ...
        for subdirname in dirnames:
            for filename in glob.glob(os.path.join(dirname, subdirname)+'/*settings*.cfg'):
                ...
                if ('DTX' in filename) and (sleep_alignment == 'none'):
                    filedata = np.genfromtxt(os.path.join(dirname, subdirname)+'/delivered_final.csv', delimiter=',')
                    index = sequential_index 
                    result[sweep_values.index(users), index, dep] = filedata
                    if not np.isnan(filedata) and not filedata == 0:
    ...
    np.savetxt(outpath+'/delivered_sumrates.csv', result, delimiter=',')
