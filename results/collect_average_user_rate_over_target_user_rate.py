...
'''Collect the average user rate over the target rate 
y-axis: average user rate
x-axis: target rate
...
def main(searchpath, outpath):
    ...
    for dt in np.arange(1,data_types):
        for si, sv in enumerate(sweep_values):
            elements = 100*result[dt][si] # times 100 for users and timeslots per frame

            # percentage at last iteration
            avg_rates[dt,si] = np.mean(elements) # should be around the value of sv

    # x-axis
    avg_rates[0,:] = sweep_values

    target = outpath + '/average_user_rates_over_target_rate.csv'
