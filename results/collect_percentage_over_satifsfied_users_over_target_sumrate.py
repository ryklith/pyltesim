...
def main(searchpath, outpath):
    ...
    for dt in np.arange(1,data_types):
        for si, sv in enumerate(sweep_values):
            elements = 100*result[dt][si]/sv # times 100 for users and timeslots per frame
            # percentages for each iteration
            mean_el = np.sum((elements>1),axis=1)/float(elements.shape[1])
            target = outpath + 'percentage_satisfied_over_iters_' + data_str[dt] + '_' + str(sv) + '.csv'
            np.savetxt(target, mean_el, delimiter=',')

            # percentage at last iteration
            perc_tot[dt,si] = (elements[-1,:] > 1).sum()/float(len(elements[-1,:]))

    # x-axis
    perc_tot[0,:] = sweep_values

    target = outpath + '/percentage_satisfied_over_target_rate.csv'
