...
Several curves, one for each technique
...
def plot_cdf_from_file(searchpath, rate):
    """Open four files for the four techniques. Scan for rate in filename. Plot cdf."""

    import glob
    for filename in glob.glob(searchpath+'/delivered_per*'+rate+'.csv'):
        print filename
        if 'none' in filename:
    ...
    data = np.empty([4,1000])
    data[0,:] = data_none[-1,:]
    data[1,:] = data_rand[-1,:]
    data[2,:] = data_sinr[-1,:]
    data[3,:] = data_dtxs[-1,:]

    label = ['none', 'rand', 'sinr', 'dtxs']
    cdf_plot(data, '-', label=label)
#    plt.xlabel(xlabel)
#    plt.ylabel(ylabel)
#    plt.title(title)
    plt.savefig(searchpath+'/comparative_cdf_user_rates.pdf', format='pdf')
    plt.savefig(searchpath+'/comparative_cdf_user_rates.png', format='png')
...
if __name__ == '__main__':
    ...
    rate = sys.argv[2]
    plot_cdf_from_file(searchpath, rate)
