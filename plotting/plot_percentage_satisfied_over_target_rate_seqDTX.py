...
''' Plot percentage of satisfied users over target rate 
x axis: target rate per user 
...
def plot(filename):
    ...
    # data comes in a csv
    data = np.genfromtxt(filename, delimiter=',')*100 # 100 for percent
...
#    ax1.plot(x, data[7], '-kp', label='Static Reuse 3', markersize=10)
    ...
    ylabel = 'Percentage of satisfied users'
